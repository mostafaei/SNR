...
class ExerciseTopo(Topo):
	...
	#print("***links[0]***", links[0])
        for link in links:
            if link['node1'][0] == 'h':
		#print("link['node1'][0]", link['node2'][2])
                host_links.append(link)
            else:
                switch_links.append(link)
        #print("switch_links: ", switch_links)
		#print("node1:", link['node1'], " node2: ", link['node2'] )
	...
	#print("***switch_links***", switch_links)
	    ...
	    #print(sw1_name, sw1_port,  " ****parse**** ", sw2_name, sw2_port)
	...
	#print ("sw_name: ", sw_name, "sw_port: ", sw_port)
        try:
            sw_port = int(sw_port[1:])
	    #print("try: sw_port: ", sw_port)
        except:
...
class ExerciseRunner:
	...
	#we need to run processes on each hosts to measure flow completion time or FCT
	#we use iperf3 with -J option to get FCTs

	#h1 = self.net.get("h1")
	#h2 = self.net.get("h2")
	#h1.cmd("iperf3 -s &")
	#h2.cmd("iperf3 -c 10.0.1.1 -n 4000 -J > /home/p4/tutorials/utils/h2.json &" )

	#print("net.get", h.intfs.values())
	a = ["2", "3", "10","11" ]
	c=0
	for x in range(len(a)):
			...
			c+=1
				...
				i="h"+str(int(a[x]))
            			j="h"+str(int(a[y]))
				print(i,j)
				i=self.net.getNodeByName(str(i))
				j=self.net.getNodeByName(str(j))
            			#print(i,j)
            			for d in range(1):
					#c+=1
                			#i.popen("iperf3 -s -p 500%d &" %(d) , shell=True)
					i.popen("iperf3 -s -p 5001 &", shell=True)
					#print("iperf3 -s -p 500%d &" %(c))
				for d in range(1):
					#j.popen("iperf3 -c 10.0.%s.%s -n 4000 -J -p  500%d > /home/p4/tutorials/utils/iperf/h%s%s.json &" %(a[x],a[x],d,a[x],a[y]), shell=True)
					j.popen("iperf3 -c 10.0.%s.%s -n 4000 -J -p  5001 > /home/p4/tutorials/utils/iperf/h%s%s.json &" %(a[x],a[x],a[x],a[y]), shell=True)
					#print("iperf3 -c 10.0.%s.%s -n 4000 -J -p  500%d >> /home/p4/tutorials/utils/iperf/h%s%s.json &" %(a[x],a[x],c,a[x],a[y]))
    ...
    def parse_links(self, unparsed_links):
        ...
        """
        #print("unparsed_links: ", unparsed_links)
        links = []
