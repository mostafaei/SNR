...
def get_firstnode_RankBased():
    ...
    for node in nodes:
        ...
        for n_node in neighbors_of_Node:
            ...
            sum2 += cost
            count+=1
        lst_nodes_avg_wgt = lst_nodes_avg_wgt.append({"node": node, "latency": sum, "bw": sum1,
                                                     "cost": sum2},ignore_index=True)
    ...
    for index, row in lst_nodes_avg_wgt.iterrows():
        x = w1*(row.bw/bw_max)
        y = w2*(lat_min/row.latency)
        z = w3*(cost_min/row.cost)
...
def subSet_of_Nodes(Num_Of_Nodes, get_firstnode):
    ...
    for n in nodes:
        ...
        ranktable = rank(currentnode, lst_Neighbors)
...
startNode=str(get_firstnode_RankBased())
